...
export default function ServiceCard({ servicio, onClick }) {
  const getImageByTitle = (title) => {
    ...
    if (lower.includes('boda') || lower.includes('wedding') || lower.includes('mariage') || lower.includes('hochzeit') || lower.includes('bruiloft')) {
      return 'https://images.pexels.com/photos/169198/pexels-photo-169198.jpeg';
    }
    if (lower.includes('cumpleaños') || lower.includes('birthday') || lower.includes('anniversaire') || lower.includes('geburtstag') || lower.includes('verjaardag')) {
  ...
  return (
    <div className="service-card" onClick={() => onClick(servicio)}>
      <img
        src={getImageByTitle(servicio.title)}
        alt={servicio.title}
